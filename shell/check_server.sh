...
uploadHostDailyCheckApi="http://192.168.77.16:8000/upload/"
uploadHostDailyCheckReportApi="http://192.168.77.16:8000/record/"
...
RESULTFILE="$LOGPATH/HostDailyCheck-`hostname`-`date +%Y%m%d-%H`.txt"
PUTFILENAME="HostDailyCheck-`hostname`-`date +%Y%m%d-%H`.txt"


#定义报表的全局变量
report_DateTime="-1"    #日期 ok
report_Hostname="-1"    #主机名 ok
report_OSRelease="-1"    #发行版本 ok
report_Kernel="-1"    #内核 ok
report_Language="-1"    #语言/编码 ok
report_LastReboot="-1"    #最近启动时间 ok
report_Uptime="-1"    #运行时间（天） ok
report_CPUs="-1"    #CPU数量 ok
report_CPUType="-1"    #CPU类型 ok
report_Arch="-1"    #CPU架构 ok
report_MemTotal="-1"    #内存总容量(MB) ok
report_MemFree="-1"    #内存剩余(MB) ok
report_MemUsedPercent="-1"    #内存使用率% ok
report_DiskTotal="-1"    #硬盘总容量(GB) ok
report_DiskFree="-1"    #硬盘剩余(GB) ok
report_DiskUsedPercent="-1"    #硬盘使用率% ok
report_InodeTotal="-1"    #Inode总量 ok
report_InodeFree="-1"    #Inode剩余 ok
report_InodeUsedPercent="-1"    #Inode使用率 ok
report_IP="-1"    #IP地址 ok
report_MAC="-1"    #MAC地址 ok
report_Gateway="-1"    #默认网关 ok
report_DNS="-1"    #DNS ok
report_Listen="-1"    #监听 ok
report_Selinux="-1"    #Selinux ok
report_Firewall="-1"    #防火墙 ok
report_USERs="-1"    #用户 ok
report_USEREmptyPassword="-1"   #空密码用户 ok
report_USERTheSameUID="-1"      #相同ID的用户 ok 
report_PasswordExpiry="-1"    #密码过期（天） ok
report_RootUser="-1"    #root用户 ok
report_Sudoers="-1"    #sudo授权  ok
report_SSHAuthorized="-1"    #SSH信任主机 ok
report_SSHDProtocolVersion="-1"    #SSH协议版本 ok
report_SSHDPermitRootLogin="-1"    #允许root远程登录 ok
report_DefunctProsess="-1"    #僵尸进程数量 ok
report_SelfInitiatedService="-1"    #自启动服务数量 ok
report_SelfInitiatedProgram="-1"    #自启动程序数量 ok
report_RuningService="-1"           #运行中服务数  ok
report_Crontab="-1"    #计划任务数 ok
report_Syslog="-1"    #日志服务 ok
report_SNMP="-1"    #SNMP  OK
report_NTP="-1"    #NTP ok
report_JDK="-1"    #JDK版本 ok
function version(){
    echo ""
    echo ""
...
function uploadHostDailyCheckReport(){
    json="{
        ...
        \"o_s_release\":\"$report_OSRelease\",
        \"kernel\":\"$report_Kernel\",
        \"language\":\"$report_Language\",
        \"last_reboot\":\"$report_LastReboot\",
        \"uptime\":\"$report_Uptime\",
        \"c_p_us\":\"$report_CPUs\",
        \"c_p_u_type\":\"$report_CPUType\",
        \"arch\":\"$report_Arch\",
        \"mem_total\":\"$report_MemTotal\",
        \"mem_free\":\"$report_MemFree\",
        \"mem_used_percent\":\"$report_MemUsedPercent\",
        \"disk_total\":\"$report_DiskTotal\",
        \"disk_free\":\"$report_DiskFree\",
        \"disk_used_percent\":\"$report_DiskUsedPercent\",
        \"inode_total\":\"$report_InodeTotal\",
        \"inode_free\":\"$report_InodeFree\",
        \"inode_used_percent\":\"$report_InodeUsedPercent\",
        \"i_p\":\"$report_IP\",
        \"m_a_c\":\"$report_MAC\",
        ...
        \"u_s_e_rs\":\"$report_USERs\",
        \"u_s_e_r_empty_password\":\"$report_USEREmptyPassword\",
        \"u_s_e_r_the_same_u_i_d\":\"$report_USERTheSameUID\",
        \"password_expiry\":\"$report_PasswordExpiry\",
        \"root_user\":\"$report_RootUser\",
        \"sudoers\":\"$report_Sudoers\",
        \"s_s_h_authorized\":\"$report_SSHAuthorized\",
        \"s_s_h_d_protocol_version\":\"$report_SSHDProtocolVersion\",
        \"s_s_h_d_permit_root_login\":\"$report_SSHDPermitRootLogin\",
        \"defunct_prosess\":\"$report_DefunctProsess\",
        \"self_initiated_service\":\"$report_SelfInitiatedService\",
        \"self_initiated_program\":\"$report_SelfInitiatedProgram\",
        \"runing_service\":\"$report_RuningService\",
...
#上传检查结果的文件
curl -X PUT -F "filename=@$RESULTFILE" "$uploadHostDailyCheckApi$PUTFILENAME" 2>/dev/null
